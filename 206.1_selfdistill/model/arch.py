...
class DMVFN(nn.Module):
    def __init__(self):
        ...
        self.bake_time = TimeBlock(2 + 11 + 1, 2)

    def forward(self, x, t_pred, scale, training=True):
        batch_size, _, height, width = x.shape  # 8, 15, 256, 256
        ' --- get img'
        ...
        if training:
            warped_fea2 = fea2
            flow_teacher = Variable(torch.zeros(batch_size, 4, height, width)).cuda()
            distill_flow_ls = []
            seq1_teacher = torch.cat(
                (
            ...
            seq2_teacher = torch.cat(
                (
                    torch.unsqueeze(fean1, dim=1),
                    torch.unsqueeze(fea0, dim=1),
                    torch.unsqueeze(fea1, dim=1),
                    torch.unsqueeze(fea2, dim=1)
                ),
                dim=1
            )   # (B, 4, 11, 256, 256)
            seq1_teacher, seq2_teacher = self.convlstm(seq1_teacher)[-1][-1][0], self.convlstm(seq2_teacher)[-1][-1][0]   # (B, 8, 256, 256), (B, 8, 256, 256)
        ' --- multi input'
        seq0 = torch.cat(
            (
        ...
        seq0, seq1 = self.convlstm(seq0)[-1][-1][0], self.convlstm(seq1)[-1][-1][0]   # (B, 8, 256, 256), (B, 8, 256, 256)
        merged_ls = []
        mask_ls = []
        refine_ls = []
        ...
        for i in range(9):
            ...
            ' --- self distill'
            if training:
                flow_teacher_d, _, _ = stu[i](
                    torch.cat(
                        (fea1, fea2, warped_fea1, warped_fea2), 1
                    ),
                    flow_teacher,
                    scale=scale[i]
                )
                flow_teacher = flow_teacher + flow_teacher_d
                flow_teacher_delta_1 = self.bake_time(
                    torch.cat(
                        (
                            flow_teacher[:, : 2].clone(),
                            seq1_teacher,
                            (torch.ones(batch_size, 1, height, width) * t_pred).to(device)
                        ),
                        dim=1
                ...
                flow_teacher_delta_2 = self.bake_time(
                    torch.cat(
                        (
                            flow_teacher[:, 2: 4].clone(),
                            seq2_teacher,
                            (torch.ones(batch_size, 1, height, width) * (t_pred - 1)).to(device)
                        ),
                        dim=1
                ...
                flow_teacher = flow_teacher + torch.cat((flow_teacher_delta_1, flow_teacher_delta_2), dim=1)
            warped_fea0 = warp(fea0, flow[:, : 2])
            warped_fea1 = warp(fea1, flow[:, 2: 4])
            ' --- self distill'
            if training:
                distill_flow_ls.append(
                    (flow[:, 2: 4], flow_teacher[:, : 2])
                )
                warped_fea2 = warp(fea2, flow_teacher[:, 2: 4])
        ...
        if training:
            return merged_ls, distill_flow_ls
        else:
            return merged_ls
